<template>
    <div class="flex h-screen container mx-auto relative">
        ...
        >
            ...
            <button class="p-3 hover:bg-gray-50 w-full text-left text-sm" @click="onLoginout">
                {{ currentUser.id }}계정에서 로그아웃
            </button>
        </div>
...
export default {
    ...
    setup() {
        ...
        onBeforeMount(() => {
            routes.value = router.options.routes.filter((route) => route.meta.isMenu == true) || 'DefaultLayout';
        });
