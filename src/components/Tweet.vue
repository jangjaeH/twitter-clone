        ...
        <div class="ml-3 flex-1 flex flex-col space-y-1">
            <div class="text-sm flex justify-between items-center">
                <div class="space-x-2">
                    ...
                    <span>·</span>
                    <span class="text-graqy-500 text-xs">{{ moment(tweet.create_at).fromNow() }}</span>
                </div>
            ...
            <div class="flex justify-between">
                <div @click="showCommentModal = true" class="text-gray-500 hover:text-primary">
                    ...
                    <span class="ml-1 textgray-500 text-sm">{{ tweet.num_comments }}</span>
                </div>
                <div v-if="!tweet.isRetweeted" class="text-gray-500 hover:text-green-400" @click="handleRetweet(tweet)">
                    ...
                    <span class="ml-1 textgray-500 text-sm">{{ tweet.num_retweets }}</span>
                </div>
                <div v-else class="text-green-400" @click="handleRetweet(tweet)">
                    <i class="fas fa-retweet hover:bg-blue-50 rounded-full p-2"></i>
                    <span class="ml-1 textgray-500 text-sm">{{ tweet.num_retweets }}</span>
                </div>
                <div v-if="!tweet.isLiked" class="text-gray-400 hover:text-read-400" @click="handleLikes(tweet)">
    ...
    setup() {
        ...
        const onDeleteTweet = async (tweet) => {
            if (confirm('정말로 트윗을 삭제하시겠습니까?')) {
                ...
                // delete likes
                const liekSnapshot = await LIKE_COLLECTION.where('from_tweet_id', '==', tweet.id).get();
                liekSnapshot.docs.forEach(async (doc) => await doc.ref.delete());
                // delete retweets
