...
export default {
  setup() {
    ...
    onBeforeMount(async () => {
      const queryResult = await USER_COLEECTION.orderBy(
        "created_at",
        "desc"
      ).get();
      ...
      const handleOnSnapshot = (snapshot) => {
        snapshot.docChanges().forEach(async (change) => {
          if (change.type === "added") {
            pushMessage.value.splice(change.newIndex, 0, change.doc.data());
      ...
      MESSAGE_COLLECTION.where("to_uid", "==", currentUser.value.uid)
        .orderBy("created_at", "desc")
        .onSnapshot(handleOnSnapshot);
    });
